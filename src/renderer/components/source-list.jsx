...
export default class SourceFiles extends React.Component {
  ...
  render() {
    ...
    return (
      ...
      <div style={{border:"1px solid #000",marginTop:"5px",padding:"3px",height:"300px",overflow:"scroll"}}>
      <ul>
        {files.map((file) => <Item key={file.filename} selected={file.selected} filename={file.filename} onChange={this.handleSelection}/>)}
      </ul>
      </div>
    </div>
    );
